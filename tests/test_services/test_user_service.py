...
async def test_create_user_with_valid_data(db_session, email_service):
    ...
    assert user.email == user_data["email"], "User email should match the input email"
...
async def test_create_user_with_invalid_data(db_session, email_service):
    ...
    user = await UserService.create(db_session, user_data, email_service)
    assert user is None, "User creation should fail with invalid data"

# Test fetching a user by ID when the user exists
async def test_get_by_id_user_exists(db_session, user):
    retrieved_user = await UserService.get_by_id(db_session, user.id)
    assert retrieved_user.id == user.id, "Fetched user should match the given user ID"
...
async def test_get_by_id_user_does_not_exist(db_session):
    ...
    assert retrieved_user is None, "Non-existent user should return None"
...
async def test_get_by_nickname_user_exists(db_session, user):
    ...
    assert retrieved_user.nickname == user.nickname, "Fetched user should match the given nickname"
...
async def test_get_by_email_user_exists(db_session, user):
    ...
    assert retrieved_user.email == user.email, "Fetched user should match the given email"
...
async def test_update_user_valid_data(db_session, user):
    ...
    assert updated_user.email == new_email, "User's email should be updated correctly"

# Test updating a user with invalid data
async def test_update_user_invalid_data(db_session, user):
    updated_user = await UserService.update(db_session, user.id, {"email": "invalidemail"})
    assert updated_user is None, "User update should fail with invalid data"

# Test deleting a user who exists
async def test_delete_user_exists(db_session, user):
    deletion_success = await UserService.delete(db_session, user.id)
    assert deletion_success is True, "User should be deleted successfully"
...
async def test_delete_user_does_not_exist(db_session):
    ...
    assert deletion_success is False, "Deletion should fail for non-existent user"
...
async def test_list_users_with_pagination(db_session, users_with_same_role_50_users):
    ...
    assert users_page_1[0].id != users_page_2[0].id, "Users on different pages should have different IDs"
...
async def test_register_user_with_invalid_data(db_session, email_service):
    ...
    user = await UserService.register_user(db_session, user_data, email_service)
    assert user is None, "User registration should fail with invalid data"
...
async def test_login_user_successful(db_session, verified_user):
    ...
    assert logged_in_user is not None, "Login should be successful with correct credentials"
...
async def test_login_user_incorrect_email(db_session):
    ...
    assert user is None, "Login should fail with incorrect email"
...
async def test_login_user_incorrect_password(db_session, user):
    ...
    assert user is None, "Login should fail with incorrect password"
...
async def test_reset_password(db_session, user):
    ...
    assert reset_success is True, "Password reset should be successful"
...
async def test_verify_email_with_token(db_session, user):
    ...
    assert result is True, "Email verification should succeed with a valid token"
