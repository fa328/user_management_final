...
async def get_user(user_id: UUID, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    ...
    return UserResponse.model_construct(
        ...
        github_profile_url=create_user.github_profile_url,
        linkedin_profile_url=create_user.linkedin_profile_url,
        role=user.role,
        email=user.email,
...
async def update_user(user_id: UUID, user_update: UserUpdate, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    ...
    if updated_user == 'email_exist':
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Email aleady exist")
    if not updated_user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
...
@router.put("/users/enhanceMyProfile/", response_model=UserResponse)
...
@router.patch("/users/{user_id}/Improve", response_model=UserResponse)
